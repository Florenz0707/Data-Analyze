...
| M7     | 可观测性、部署和可靠性         |     P1 | M2、M3、M6 | 进行中（日志、追踪、指标、健康检查与持久化日志完成）                    | Codex  | 2026-09-03 | E-038～E-040 |
...
```text
状态：进行中（日志、追踪、指标、健康检查与持久化日志子任务完成；部署和故障演练待执行）
负责人：Codex
更新时间：2026-09-03
...
当前实测：后端指标/健康/日志/API 定向测试 47/47；前端 Vitest 26/26；普通响应和流式响应均验证响应头与链路上下文；日志脱敏覆盖 Bearer、API Key、密码、嵌套字段和 request 对象；指标端点、Readiness 503 和 Provider 不加载模型均有回归
剩余工作：集中式日志/指标采集、采样/保留策略、告警规则、Provider 分布式传播、真实队列/多 Worker 指标、部署回滚和故障演练；真实 Token usage 仍需 Provider 计费字段接入
验收证据：E-038～E-040、`doc/m7_logging_and_tracing.md`、`doc/m7_metrics_and_health.md`、`doc/m7_persistent_structured_logging.md`
```
