...
| ------ | ------------------------------ | -----: | ---------- | ------ | ------ | ---------- | -------- |
| M0     | 建立可复现基线与评测集         |     P0 | 无         | 进行中 | Codex  | 待人工复核 | -        |
...
| 评测查询数         |                      ≥ 50 |                                   50 | 通过               |
| 负样本比例         |                     ≥ 20% |                                  20% | 通过               |
| 双人独立标注覆盖   |                     ≥ 20% | 20%（项目角色口径；实际执行者含 AI） | 通过（冲突待裁决） |
| 评测集字段完整率   |                      100% |                                 100% | 通过               |
| 同环境重复运行差异 | 检索指标差异 ≤ 1 个百分点 |                           0 个百分点 | 通过               |
| 基线报告完整率     |                      100% |         自动化部分完成，人工评分待补 | 未通过             |
...
本轮复核：用户已审核 10 条并确认无问题；按用户指定，Codex 作为第二评审角色对同 10 条完成独立复核并记录原因/证据/步骤评分。由于用户未提供结构化分数，而第二评审角色发现多条无证据诊断，已登记为待用户裁决的结论差异
复核结论：10 条负样本中 9 条未拒答且存在无证据确定性诊断/高风险建议；W010 仅部分拒答，仍有臆测和凭证重置建议
剩余工作：由用户裁决 10 条冲突并补充结构化分数；如用于生产级质量声明，仍需真实第二名人类评审
关键发现：Recall@10=0.95；负样本明确拒答率=0%，存在无证据仍生成的问题
验收证据：E-001、`evaluation/m0/evidence/double_review_results.json`
```
...
| 无证据正确拒答率    |       0% |    ≥ 90% |       0% | 10 条 Windows 负样本，按明确拒答标记 | E-001 | 2026-08-28 |
...
| 索引构建总耗时   |       28.06s |     记录趋势 |       28.06s | 1708 条，batch=32，临时目录                 | E-001 | 2026-08-28 |
...
| 后端测试               |       全部通过 | 系统检查通过，0 tests | `manage.py test`；当前尚无测试用例 | E-001        | 2026-08-28 |
...
| ----- | ------ | -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------- | ------ | ---------- |
| E-001 | M0     | 基线报告             | `evaluation/m0/baseline_report.md`、`evaluation/m0/evidence/`、`evaluation/m0/evidence/double_review_results.json`、`evaluation/m0/evidence/double_review_annotations.csv` | 自动化基线通过；10 条完成双评审角色覆盖，存在待裁决冲突    | Codex  | 2026-08-28 |
