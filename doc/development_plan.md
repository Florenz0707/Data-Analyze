...
| M4     | 重构数据摄取、索引版本与检索   |     P1 | M0、M1     | 进行中（数据清洗与文档构建完成）                    | Codex  | TODO       | -          |
...
负责人：Codex
更新时间：2026-08-31
已完成：统一日志 Schema、确定性 CSV 解析、编码/空值/级别归一化、重复记录处理、敏感信息隔离、领域文档模板、稳定 ID、Metadata、分块、Manifest 差异和批量流式构建
剩余工作：索引版本、原子切换/回滚、状态查询、Metadata 过滤、相似度阈值、混合召回、Reranker 和 M4 评测
验收证据：E-024、`doc/m4_data_cleaning_and_document_building.md`、`evaluation/m4/evidence/data_quality_report.json`
```
...
| R-014 | Chroma 文档未保存源文件、行号和稳定日志 ID metadata                       |   高 |   中 | M4 文档构建已生成稳定 ID、source_file/source_row 和版本 Metadata；现有旧集合尚未重建，待 M4 索引版本切换接入                      | Codex  | 部分缓解 |
...
严格按依赖关系，下一步完成 M4 的索引版本和增量切换；M3 集成验收仍需单独登记；不要直接修改 Agent 或界面功能。
