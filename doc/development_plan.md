...
| M3     | 完成认证、安全和自定义模型闭环 |     P0 | M1、M2     | 进行中（Token 生命周期、限流、外部模型闭环已完成）  | Codex  | TODO       | -          |
...
剩余工作：SSRF 防护和 M3 集成验收
验收证据：E-020、E-021、E-022、`doc/m3_token_lifecycle.md`、`doc/m3_rate_limiting.md`、`doc/m3_external_model_closure.md`
更新时间：2026-08-31
```
...
| E-022 | M3     | 外部模型闭环报告                 | `doc/m3_external_model_closure.md`、`deepseek_api/models.py`、`deepseek_api/services.py`、`deepseek_api/api.py`、`deepseek_api/migrations/0012_external_model_closure.py`、`deepseek_api/tests/test_services.py`、`deepseek_api/tests/test_api.py`、`pyproject.toml`、`uv.lock`                                                                                                                                       | 后端全量测试 77/77；外部配置用户范围解析、稳定外键、真实生成构造参数、密钥加密/更新、别名冲突、删除回退和响应脱敏测试通过；迁移检查无变化，0012 已应用到当前 PostgreSQL；SSRF 仍待后续                | Codex  | 2026-08-31 |
...
| R-005 | 外部接口密钥和 SSRF                                                       |   高 |   高 | API Key 已在 0012 中加密并补充脱敏测试；仍需完成 Base URL 协议、地址解析、重定向和 SSRF 防护                                      | Codex  | 部分缓解 |
...
严格按依赖关系，下一步完成 M3 的 SSRF 防护和集成验收；不要直接修改 Agent 或界面功能。
