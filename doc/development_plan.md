...
| M2     | 修复正确性、并发与数据一致性   |     P0 | M1         | 进行中（模型实例隔离、Session/History 一致性完成） | Codex  | TODO       | -          |
...
```text
状态：进行中（模型实例隔离、Session/History 一致性、缓存正确性及错误语义完成）
负责人：Codex
更新时间：2026-08-30
...
剩余工作：实际限流接入、错误重试/熔断和真实部署并发/共享缓存压测等 M2/M3 子任务
验收证据：E-013、E-014、E-015、E-016、E-017、`doc/m2_model_instance_isolation.md`、`doc/m2_session_history_consistency.md`、`doc/m2_cache_correctness.md`、`doc/m2_error_semantics.md`、`deepseek_api/migrations/0007_session_history_foreign_key.py`、`deepseek_api/migrations/0008_session_user_and_history_ordering.py`
```
...
| 后端测试                |       全部通过 |             57/57 通过 | `DJANGO_TESTING=true`；独立测试数据库；无真实模型/网络                    | E-017        | 2026-08-30 |
...
| E-003 | M2     | 并发与数据迁移报告               | `doc/m2_session_history_consistency.md`（数据一致性部分）；真实多连接压测待补充                                                                                                                                                                                                                                                | 用户外键、基础并发写入、复合游标、缓存正确性和错误语义子任务已验收，真实部署并发/共享缓存压测与实际限流接入仍待验收                                                                           | Codex  | 2026-08-30 |
