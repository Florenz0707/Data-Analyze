...
| M2     | 修复正确性、并发与数据一致性   |     P0 | M1         | 进行中（模型实例隔离完成） | Codex  | TODO       | -          |
...
负责人：Codex
更新时间：2026-08-29
已完成：建立 Ruff、ESLint、Prettier 与 pre-commit 质量门禁；统一后端配置路径、RESPONSE_TOP_K、Prompt 占位符、Django 环境变量和测试模式；新增后端 43 项测试、前端 14 项单元测试、1 项 Mock E2E；加入 Fake LLM/Embedding、临时 Chroma、配置校验/脱敏摘要、前端环境变量和 GitHub Actions 质量门禁
当前实测：后端测试 43/43；核心后端覆盖率 87%（API 100%，服务 78%，配置 85%）；前端 API 100%、Store 92.28%、总体 94.02%；Mock E2E 1/1；lint、格式检查、构建和 pre-commit 全部通过
...
负责人：Codex
更新时间：2026-08-29
已完成：移除请求期全局 LlamaIndex 模型覆盖；显式传递 LLM/Embedding；新增线程安全有界实例缓存；用户模型名传入 Provider；缓存键加入 Provider、模型、endpoint 与版本；Fake Provider 并发隔离测试通过
剩余工作：Session/History 外键与迁移、删除一致性、事务、错误码和限流等 M2 子任务
验收证据：E-013、`doc/m2_model_instance_isolation.md`、`deepseek_project/model_runtime.py`、`deepseek_project/tests/test_model_runtime.py`
```
...
| --------------------- | -------: | ----: | -------: | ------------------------------------- | ---------- |
| 并发模型串台          |   未测试 |     0 |        0 | E-013（50 个 Fake Provider 并发请求） | 2026-08-29 |
...
| 后端测试                |       全部通过 |             43/43 通过 | `DJANGO_TESTING=true`；独立测试数据库；无真实模型/网络 | E-002        | 2026-08-29 |
| 后端核心模块行覆盖率    |          ≥ 80% |                    87% | 43 tests；排除 migrations/tests/asgi/wsgi              | E-002        | 2026-08-29 |
| 后端关键 API 覆盖率     |           100% |                   100% | `deepseek_api/api.py` 正常、异常、认证与参数分支       | E-002        | 2026-08-29 |
