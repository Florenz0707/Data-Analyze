...
| M8     | Agent Workflow 与只读工具      |     P2 | M5、M7     | 进行中（固定 Workflow、评测适配器和 50 任务离线基线完成，生产适配器待实现）     | Codex  | TODO       | E-044～E-046 |
...
当前状态：M5 的非实现者人工复核已完成；M2 已有 Redis 多进程同 Key 证据 E-043，但数据库并发、故障恢复和生产级并发边界仍待验收；M8 权限与审计方案已确认，固定 Workflow、可重复评测适配器及 50 条离线任务基线已完成，生产五类只读数据源适配器、审计保留/检索和完整演练仍待验收。
...
- [x] 定义状态 Schema（`ToolContext`、`ToolRequest`、`ToolResult`）；
- [x] 定义分类、计划、执行、验证和回答节点；
- [x] 定义最大步数和时间预算；Token 预算待补；
- [x] 定义无新证据终止条件；
- [x] 定义失败、超时和人工接管路径；
- [x] 每个工具调用带请求/追踪 ID 并输出审计事件；完整节点重放待补；
- [x] 固定 Workflow 与 Agent 决策边界明确。
...
| E-044 | M8     | 固定 Workflow 核心与只读工具安全边界 | `backend/django_backend/deepseek_project/agent_workflow/core.py`、`backend/django_backend/deepseek_project/tests/test_agent_workflow.py`、`doc/m8_tool_security_plan.md`                                                                                                                                                                                                                                                                                                                                                                                                                                          | 固定分类/计划/执行/验证/回答流程；五个工具白名单；角色、服务、参数、身份覆盖、步数、超时和响应大小校验；结构化审计事件；定向后端测试 6/6；默认未配置适配器返回 BACKEND_UNAVAILABLE，不伪造证据；真实数据源适配器和完整演练待补                                                                                                    | Codex  | 2026-09-04 |
| E-045 | M8     | 固定 Workflow 离线安全评测           | `evaluation/m8/run_workflow_evaluation.py`、`evaluation/m8/evidence/workflow_offline.json`、`doc/m8_tool_security_plan.md`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | 50 个固定任务全部通过：40 个正常只读任务和 10 个未知工具/越权/身份覆盖/非法参数/预算任务；耗时约 0.01s；仅验证离线契约与安全边界，不代表真实数据源可用                                                                                                                                                                            | Codex  | 2026-09-04 |
| E-046 | M8     | 五类只读评测适配器与审计预算回归     | `backend/django_backend/deepseek_project/agent_workflow/adapters.py`、`backend/django_backend/deepseek_project/agent_workflow/core.py`、`backend/django_backend/deepseek_project/tests/test_agent_workflow.py`                                                                                                                                                                                                                                                                                                                                                                                                    | `InMemoryReadOnlyDataSource` 覆盖日志、指标、发布、依赖和事故筛选；Workflow 增加 Token 预算、有限审计索引和拒绝/无证据/超时接管；定向后端测试 8/8 通过；生产数据源和集中审计存储仍待接入                                                                                                                                          | Codex  | 2026-09-04 |
...
| 2026-09-04 | Codex | 用户确认权限与审计方案后，实现 M8 固定 Workflow 核心、五工具注册表、Schema/权限/预算校验、结构化审计和无证据人工接管，并补充 5 项定向测试；真实数据源适配器待补 | M8 | E-044、`doc/m8_tool_security_plan.md` |
| 2026-09-04 | Codex | 新增 M8 固定 Workflow 离线评测入口，使用 6 个固定用例验证有效查询、五工具链路和越权/身份覆盖/时间范围拒绝；6/6 通过 | M8 | E-045、`evaluation/m8/evidence/workflow_offline.json` |
| 2026-09-04 | Codex | 补齐 M8 可重复只读评测适配器、Token/审计预算和 50 条离线任务评测；40 条正常任务与 10 条安全边界任务全部通过，生产数据源和集中审计存储留待部署接入 | M8 | E-045、E-046、`evaluation/m8/evidence/workflow_offline.json` |
...
下一步按 E-030 和 M8 前置条件逐项收敛：M5 非实现者人工复核已完成，保留 5 分制评分待补；继续完成 M2 数据库/共享缓存故障恢复与生产并发边界评估，推进 M4/M6/M7 未关闭项，并为五个只读工具接入真实查询适配器、审计保留/检索和完整故障演练。
