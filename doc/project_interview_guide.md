...
全栈层面，前端负责登录、会话、消息状态、模型选择和 Markdown 渲染；后端负责认证、会话持久化、模型适配和 RAG 调用。项目的亮点是完成了从前端到大模型服务的完整闭环，并针对硬件限制、多模型接入、多轮历史污染、索引重复构建和用户自定义模型安全做了工程优化。当前不足是检索评测、SSRF 防护和生产级安全仍需完善。
...
当前系统能够接收用户 Base URL、模型名、API Key 和别名，用最小 Chat 请求验证接口，并将加密配置按用户范围保存。用户可以通过别名或模型名选择配置，生成路径读取保存的端点、模型和密钥构造 OpenAI 兼容 Client；删除当前配置时自动回退默认模型。当前仍未完成的是对用户可控 Base URL 的 SSRF 防护。
...
- “系统支持用户在授权范围内切换内置或自定义模型。”当前已完成选择、生成和删除回退闭环，但 SSRF 防护仍未完成。
