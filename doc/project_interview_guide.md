...
全栈层面，前端负责登录、会话、消息状态、模型选择和 Markdown 渲染；后端负责认证、会话持久化、模型适配和 RAG 调用。项目的亮点是完成了从前端到大模型服务的完整闭环，并针对硬件限制、多模型接入、多轮历史污染、索引重复构建和用户自定义模型安全做了工程优化。当前不足是检索评测、出口防火墙策略、密钥托管和其他生产级安全仍需完善。
...
保存请求范围、用户/IP摘要和时间窗口内的请求次数。当前认证阶段会按接口分级调用数据库共享 bucket，超额请求返回 429 和 Retry-After。
...
保存用户自定义的 Base URL、模型名、Fernet 加密后的 API Key 和别名；API 响应只返回展示名称，生成时才在 Provider 边界解密。
...
数据库保存 `provider`、模型名以及可选的 `ExternalLLMAPI` 稳定外键；生成路径将内置模型或外部模型实例显式传给查询流程。因此用户选择的具体模型会生效，外部别名只作为当前用户范围内的解析入口。实例缓存还会纳入 Provider、模型、端点和外部密钥摘要，避免密钥轮换后复用旧客户端。
...
当前系统能够接收用户 Base URL、模型名、API Key 和别名，用最小 Chat 请求验证接口，并将加密配置按用户范围保存。用户可以通过别名或模型名选择配置，生成路径读取保存的端点、模型和密钥构造 OpenAI 兼容 Client；删除当前配置时自动回退默认模型。Base URL 已加入 HTTPS、公网 DNS/IP、固定连接、重定向、超时和响应大小限制，但生产仍需出口防火墙和真实部署演练。
...
- 当前使用 Fernet 加密保存，旧明文由 0012 迁移转换后删除；
- 生产应将 Fernet 密钥放入 KMS/Vault 或等价密钥管理服务；
- API 返回和日志不回传完整密钥；
- 更新同一模型会重新加密密钥，密钥轮换和审计仍需进一步完善。

#### SSRF

用户提交的 `base_url` 会在探测和实际生成前统一验证：默认只允许 HTTPS，并拒绝凭据、查询参数、片段、环回、链路本地、内网、元数据和 IPv4-mapped IPv6 地址；DNS 结果必须全部为公网地址，连接时固定到已验证 IP。客户端关闭环境代理和自动重定向，并设置连接/读取/写入/连接池超时及响应大小上限。当前仍需在部署层补充出口防火墙、域名白名单和真实网络演练。
...
| 外部模型已接入生成链路       | 已解决用户选择与实际调用不一致 | 稳定外键 + External Provider Resolver                         |
| 外部 API Key 明文存储        | 已通过 Fernet 加密缓解         | KMS/Vault、轮换与审计                                         |
| 用户可控 Base URL            | SSRF                           | 应用层协议、地址、DNS、重定向和响应限制；部署层仍需出口防火墙 |
...
> 数据库保存用户 Provider、具体模型和可选的外部配置稳定 ID，调用时按用户偏好动态构建/复用对应实例。内置模型和外部 OpenAI 兼容模型都显式传入查询流程，不修改全局 Settings；外部别名解析限定当前用户范围。
...
> 当前实现使用数据库共享的固定窗口 bucket，在认证阶段按登录、聊天、模型验证和通用接口分级，并同时检查用户/IP维度；超额返回 429 和窗口剩余时间 `Retry-After`。固定窗口适合作为一致性实现，但生产高吞吐场景仍应在 API Gateway/Redis 增加令牌桶或滑动窗口，并按租户和模型成本细分。
...
- “系统支持用户在授权范围内切换内置或自定义模型。”当前已完成选择、生成、删除回退和 SSRF 输入安全闭环，但生产仍需出口防火墙、密钥托管和真实部署演练。
