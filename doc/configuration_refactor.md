...
端口默认值、环境变量展开和无效远程配置；后端全量测试 62/62 通过。当前自动化
回归仍使用独立 SQLite 测试库；此外，已使用当前 PostgreSQL 配置完成 50 请求、20
worker 的模型隔离集成验证，详见 `evaluation/postgres_model_isolation.py` 和
`doc/m2_model_instance_isolation.md`。这不代表 MySQL 或多进程生产环境已经完成
验证，后续仍应使用容器化数据库做迁移、并发锁和连接故障测试。
